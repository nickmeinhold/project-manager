...
exports.onStepCreate = exports.onStepUpdate = void 0;
const admin = __importStar(require("firebase-admin"));
const functions = __importStar(require("firebase-functions"));
admin.initializeApp();
const db = admin.firestore();
// Trigger when a step is updated
exports.onStepUpdate = functions.firestore
...
exports.onStepCreate = functions.firestore
    .document("projects/{projectId}/steps/{stepId}")
    ...
    if (stepData.order === 0 &&
        ...
        try {
            await attemptAutomation(projectId, stepId, stepData, projectData.userId);
        }
        catch (error) {
            console.error("Automation error:", error);
        }
...
async function attemptAutomation(projectId, stepId, stepData, userId) {
...
async function simulateAutomation(stepData) {
...
async function handleStepCompletion(projectId, stepId, stepData, userId) {
    ...
    const stepsSnapshot = await db
        ...
        .orderBy("order", "asc")
        .get();
    const steps = stepsSnapshot.docs.map((doc) => (Object.assign({ id: doc.id }, doc.data())));
...
// Send notification
async function sendNotification(projectId, stepId, userId, notification) {
